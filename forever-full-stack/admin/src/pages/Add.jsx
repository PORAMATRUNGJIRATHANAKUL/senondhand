    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div>
        ...
        <div className='flex gap-3'>
          ...
          <div onClick={() => setSizes(prev => prev.includes("No-Size") ? prev.filter(item => item !== "No-Size") : [...prev, "No-Size"])}>
            <p className={`${sizes.includes("No-Size") ? "bg-pink-100" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>No-Size</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className='flex gap-3'>
          <div onClick={() => setColors(prev => prev.includes("Red") ? prev.filter(item => item !== "Red") : [...prev, "Red"])}>
            <p className={`${colors.includes("Red") ? "bg-red-200" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>Red</p>
          </div>
          <div onClick={() => setColors(prev => prev.includes("Blue") ? prev.filter(item => item !== "Blue") : [...prev, "Blue"])}>
            <p className={`${colors.includes("Blue") ? "bg-blue-200" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>Blue</p>
          </div>
          <div onClick={() => setColors(prev => prev.includes("Green") ? prev.filter(item => item !== "Green") : [...prev, "Green"])}>
            <p className={`${colors.includes("Green") ? "bg-green-200" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>Green</p>
          </div>
          <div onClick={() => setColors(prev => prev.includes("Black") ? prev.filter(item => item !== "Black") : [...prev, "Black"])}>
            <p className={`${colors.includes("Black") ? "bg-black text-white" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>Black</p>
          </div>
          <div onClick={() => setColors(prev => prev.includes("White") ? prev.filter(item => item !== "White") : [...prev, "White"])}>
            <p className={`${colors.includes("White") ? "bg-white" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>White</p>
          </div>
        </div>
      </div>
